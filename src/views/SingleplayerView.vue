                ...
                <section class="resources-section">
                    ...
                    <div class="resource-map-container">
                        <div class="resource-map">
                            <div class="resource-row" v-for="(row, rowIndex) in 3" :key="rowIndex"
                                :data-row-number="rowIndex">
                                <ResourceBlock
                                    v-for="(task, taskIndex) in randomTasks.slice(rowIndex * 3, (rowIndex + 1) * 3)"
                    ...
                    <div class="btn-divider">
                        <button class="visit-button" @click="showBombExample(true)"> Diffuse </button>
                        <button class="visit-button" @click="showCraftExample(true)"> Recipes </button>
                    </div>
                            ...
                            <div class="item-list">
                                <div v-for="item in this.craftingList" class="item-image-section">
                                    ...
                                    <button class="craft-btn"
                                        @click="craftSelectedItem(item.itemName, item.wood, item.water, item.stone)">
                                        Craft
                                    </button>
                                </div>
                            </div>
            ...
            <div class="example-sheet-content">
                ...
                <section class="bomb-section">
                    ...
                    <div class="bomb-container">
                        ...
                        <h3 class="smartmove-text">You can diffuse at <strong class="bold"> SmartMove 10 </strong>
                        </h3>

                        <button class="diffuse-btn" @click="showBombExample(false)"> Go back </button>
                    </div>
                </section>
            </div>
        </div>
        <div class="craft-sheet-example" :class="{ 'small-sheet': expanded, 'expanded-sheet': !expanded }"
            ...
            <div class="example-sheet-content">
                ...
                <section class="craft-section">
                    ...
                    <h3 class="smartmove-text">You can craft at <strong class="bold"> SmartMove 7 </strong></h3>
                    <button class="diffuse-btn" @click="showCraftExample(false)"> Go back </button>
...
export default {
    ...
    components: { ResourceBlock, Map, Header, EndScreen, CraftAlert, CollectAlert, ErrorAlert },
    data() {
        return {
            receivedUuid: '',
            taskList: [],
        ...
        const Game = {
            initialize: (locationName, color, groupNr, modules, data, beaconRadius, gameData) => {
                ...
                modules.forEach(module => {
                    ...
                    this.locations.push({ latitude: module.latitude, longitude: module.longitude, id: module.id });

                })
            },
            receiveBeaconResponse: (uuid) => {
                this.receivedUuid = uuid
                this.checkIfSameId(this.receivedUuid);
    ...
    methods: {
        ...
        checkIfSameId(checkedUuid) {
            const elementsWithId = document.querySelectorAll('[id]');
            elementsWithId.forEach(element => {
                if (element.id == checkedUuid && !this.elementStatus[checkedUuid] && checkedUuid !== "A20041E23F42") {
        ...
        startTimer() {
            ...
            this.timer = setInterval(() => {
                if (this.timeLeft > 0) {
                    ...
                    // alert("Too late! The bomb exploded!")
                    this.gameEnded = true;
                ...
                .then(data => {
                    ...
                    this.tasks.forEach(task => {
                        if (task.smartMoveID == id) {
                            ...
                            document.querySelector(".collect-alert-container").style.display = "flex";
        ...
        updateResourceCount(receivedResource) {
            // Check which resource collected and update count
            ...
            if (this.possibleItems.includes(receivedResource)) {
                this.diffuseItemCounts[receivedResource] = (this.diffuseItemCounts[receivedResource] || 0) + 1;
            }
        ...
        craftSelectedItem(selectedItem, wood, water, stone) {
            ...
            if (wood <= this.woodCount && water <= this.waterCount && stone <= this.stoneCount) {
                // Craft selected item, update count and show alert
                switch (selectedItem) {
                    case "pliers":
                        this.pliersCount++;
                        localStorage.setItem('pliers', this.pliersCount)
